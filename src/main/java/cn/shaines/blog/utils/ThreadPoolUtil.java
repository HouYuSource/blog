...
public class ThreadPoolUtil {
    ...
    private Runnable monitorWorker = new Runnable() {
        ...
        public void run() {
            try {
                //System.out.println("\r\n监控线程轮询开始, 发现线程池有线程数量:" + threadRefreshTimeMap.size());
                ...
                for (Map.Entry<String, Long> entry : threadRefreshTimeMap.entrySet()) {
                    ...
                    if (threadFutureMap.get(threadId).isDone()){
                        ...
                        //System.err.println("发现闲时线程,(该线程已完成任务,无需要监听)线程id:" + threadId);
                        removeIdList.add(threadId);
                    }else if (System.currentTimeMillis() - entry.getValue() > THREAD_TIME_OVER) {     // 超时60秒
                        // 线程未完成任务,有可能阻塞了,这里我们需要手动干掉该线程,否则有可能是造成资源的浪费
                        // System.out.println("发现超时线程,(该线程超时还没完成任务,需要关闭线程)线程id:" + threadId);
                ...
                //System.out.println("--监控线程结束, 线程池还剩下线程数量:" + threadRefreshTimeMap.size());
